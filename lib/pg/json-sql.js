...
function buildWhereEx(columnList, jsonWhere, where, parameters) {
  for (const name in jsonWhere) {
    ...
    if (column) {
      ...
      //where.push('one',JSON.stringify(value));
      //it is column
      ...
      if ($util.isObject(value) && !$util.isArray(value)) {
        ...
        for (const fun in value) {
          if ($WHERE_FUNCTIONS[fun]) {
            ...
            if ($LOGIC_FUNCTIONS[fun]) {
              //where.push(' test ',$LOGIC_FUNCTIONS[fun] , ' end ');
        ...
        if (value) {
          ...
          where.push(buildCompare(column, parameterName, 'like'));
        } else {
...
function buildQuery(columnList, jsonQuery) {
  ...
  if (distinctQuery) {
    for (const i in distinctQuery) {
      ...
      const column = name; //columnList[name];
      if (column) {
        output.push(name);
        continue;
      }
